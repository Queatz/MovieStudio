...
private fun jsWebGLSyncLayers(layersJson: String, playing: Boolean): Unit = js("""
    (function(layersJson, playing) {
        ...
        if (!S) {
            ...
            var pb = window.__msWebGLPendingBounds;
            if (pb) {
        ...
        for (var i = 0; i < layers.length; i++) {
            if (layers[i].kind === 'image' && !S.images[layers[i].url]) {
                (function(url) {
                    ...
                    img.onload = function() { rec.ready = true; }; // texture uploaded lazily at draw
                    img.src = url;
                })(layers[i].url);
            }
        }
...
private fun jsWebGLSetBounds(x: Double, y: Double, w: Double, h: Double): Unit = js("""
    (function(x, y, w, h) {
        var S = window.__msWebGLPreview;
        if (!S) {
